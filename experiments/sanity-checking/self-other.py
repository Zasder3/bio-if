...
def main():
    ...
    # sample 128 sequences
    np.random.seed(SEED)
    seqs = np.random.choice(seqs, 128, replace=False)
    print(f"Sequence of interest is {len(seqs[0])} amino acids long.")
    ...
    if not Path(f"analyses/{MODEL_NAME}_self_other_ddp_seed={SEED}").exists():
        Path(f"analyses/{MODEL_NAME}_self_other_ddp_seed={SEED}").mkdir(parents=True)
    df.to_csv(f"analyses/{MODEL_NAME}_self_other_ddp_seed={SEED}/seqs.csv", index=False)

    ds = Dataset.from_dict({'seq': seqs})
    ds = ds.map(lambda x: tokenizer(x['seq']))

    test_ds = Dataset.from_dict({'seq': [seqs[0]]})
    ...
    analyzer = Analyzer(
        analysis_name=f"{MODEL_NAME}_self_other_ddp_seed={SEED}",
        task=task,
        model=model,
        cpu=False,
    )
    ...
    analyzer.fit_all_factors(
        factors_name=f"{MODEL_NAME}_self_other_ddp",
    ...
    analyzer.compute_pairwise_scores(
        scores_name=f"{MODEL_NAME}_self_other_ddp",
        factors_name=f"{MODEL_NAME}_self_other_ddp",
